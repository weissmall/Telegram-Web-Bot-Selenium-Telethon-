...
def check_for_url(driver):
    ...
    while True:
        ...
        # Если чат не будет найден, то функция find_element_by_class_name
        # выдаст исключение
        try:
...
# Отправление в браузер JS скрипт, который скроллит область сообщений на
# заданное кол-во px/%
...
def scroll_to_top(
    ...
    # Словарь из сообщений и изображений
    images_dict = tg.run_get_images(
        ...
        for el in driver.find_elements(By.CLASS_NAME, 'message-list-item'):
            ...
            try:
                if not (el.get_attribute('id')) in msg_set:
                    if int(el.get_attribute('data-message-id')) in images_dict:
                        # print(images_dict[int(el.get_attribute('data-message-id'))])
                        add_src = el.find_element(By.TAG_NAME, 'img')
                        driver.execute_script(
                            "arguments[0].setAttribute('src', arguments[1])",
                            add_src,
                            images_dict[int(el.get_attribute('data-message-id'))]
                        )

                    msg_set[str(el.get_attribute('id'))] = el.get_attribute(
                        'innerHTML'
                    )

                    # el.set_attribute('src', images_dict[int(el.get_attribute('data-message-id'))])
...
def save_messages(path, filename, format, elems):
    ...
    for key in error_counter:
        print('There was {} of "{}" exceptions from {} messages'.format(
            error_counter[key], key, len(elems)
        ))
